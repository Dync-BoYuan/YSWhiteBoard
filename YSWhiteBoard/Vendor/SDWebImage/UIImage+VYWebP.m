...
static void VYFreeImageData(void *info, const void *data, size_t size) {
    free((void *)data);
}

@implementation UIImage (VYWebP)

+ (nullable UIImage *)VYsd_imageWithWebPData:(nullable NSData *)data {
    if (!data) {
        return nil;
    }
    ...
    if (!(flags & ANIMATION_FLAG)) {
        ...
        UIImage *staticImage = [self VYsd_rawWepImageWithData:webpData];
    ...
    do {
        ...
        if (iter.blend_method == WEBP_MUX_BLEND) {
            ...
            image = [self VYsd_rawWepImageWithData:iter.fragment];
        }
...
+ (nullable UIImage *)VYsd_blendWebpImageWithOriginImage:(nullable UIImage *)originImage iterator:(WebPIterator)iter {
    if (!originImage) {
        return nil;
    }
    ...
    UIImage *image = [self VYsd_rawWepImageWithData:iter.fragment];
    if (!image) {
        return nil;
    }
...
+ (nullable UIImage *)VYsd_rawWepImageWithData:(WebPData)webpData {
    ...
    CGDataProviderCreateWithData(NULL, config.output.u.RGBA.rgba, config.output.u.RGBA.size, VYFreeImageData);
    CGColorSpaceRef colorSpaceRef = CGColorSpaceCreateDeviceRGB();
